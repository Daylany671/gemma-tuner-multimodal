...
3.  **Numerical Stability on MPS**:
    ...
    - **Solution**: The training script (`gemma_tuner/models/gemma/finetune.py`) and wizard will default to using `bfloat16` (`bf16=True` in `SFTConfig`) when an MPS device is detected. If the hardware does not support `bfloat16`, it will fall back to full `float32`, and the user will be warned about increased memory usage.

## CLI Wizard Integration (`gemma_tuner/wizard/`) - ✅ COMPLETED
...
- [x] Created `gemma_tuner/models/gemma/finetune.py` with Gemma 3n LoRA trainer
...
- [x] Added Gemma routing in orchestrator: `gemma_tuner/scripts/finetune.py` now detects `gemma` models and dispatches `gemma_tuner.models.gemma.finetune`
- [x] Added environment preflight: `gemma_tuner/scripts/gemma_preflight.py` (arm64, MPS availability, bf16 probe, memory tips)
- [x] Added quick profiler: `gemma_tuner/scripts/gemma_profiler.py` (loads model, runs tiny forward, reports dtype/time/RSS)
